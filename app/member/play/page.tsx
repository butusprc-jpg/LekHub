...
export default function PlayPage(){
 ...
 useEffect(()=>{localStorage.removeItem("lekhub_member_name");localStorage.removeItem("lekhub_member_key");setItems([])
  initLIFF().then(line=>{if(line){setProfile(line.profile);setLiff(line.liff);setMessage("")}}).catch(()=>setMessage("กรุณาเปิดผ่าน LINE OA"))
 ...
 async function submit(){const activeLiff=liff;if(!profile||!activeLiff||!items.length||sending)return;setSending(true);setMessage("")
  ...
  if(error||!data?.success){setMessage(error?.message||"ส่งไม่สำเร็จ กรุณาลองใหม่");setSending(false);return}
  ...
  setItems([]);setValue("");setAmount("");setReviewing(false);setMessage(`ส่งเรียบร้อย รหัส ${data.reference_code||code}`);setSending(false)
 }
 function close(){ if(liff?.closeWindow) liff.closeWindow(); else window.location.replace("/member") }
 return <main className="play-mobile"><header className="play-title"><button aria-label="ปิดหน้าจอ" onClick={close}>×</button><h1>เลือกเลข</h1><span/></header>
  ...
  <section className="send-card"><div><b>{items.length} รายการ</b><span>รวม <strong>{total.toLocaleString()}</strong></span></div><button className="red-action" disabled={!profile||!items.length||sending||!open} onClick={()=>setReviewing(true)}>ทบทวนและส่ง LINE</button>{message&&<p className="play-message">{message}</p>}</section>
  {reviewing&&<div className="review-overlay" role="dialog" aria-modal="true"><section className="review-sheet"><h2>ทบทวนรายการ</h2><div className="review-items">{items.map((x,i)=><div key={`${x.value}-${i}`}><b>{x.value}</b><span>{x.category_label}</span><strong>{x.heart.toLocaleString()}</strong></div>)}</div><div className="review-sum"><span>{items.length} รายการ</span><b>รวม {total.toLocaleString()}</b></div><button className="red-action" disabled={sending} onClick={submit}>{sending?"กำลังส่ง...":"ยืนยันส่ง LINE"}</button><button className="review-back" disabled={sending} onClick={()=>setReviewing(false)}>กลับไปแก้ไข</button></section></div>}
 </main>
}
