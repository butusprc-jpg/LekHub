...
const DIRECT_PAGES=new Set(['home','booking','event','member','game','rewards']);
function initialDirectPage(){
 const q=new URLSearchParams(window.location.search).get('page');
 return DIRECT_PAGES.has(q)?q:'home';
}
...
const LIFF_ID=(import.meta.env.VITE_LINE_LIFF_ID||'2011209232-K5q7J6i1').trim();
function cachedLineProfile(){try{return JSON.parse(localStorage.getItem('baan_tonkla_line_profile')||'null')}catch(e){return null}}
async function loadLineProfile(){
 ...
 try{
  ...
  localStorage.setItem('baan_tonkla_line_profile',JSON.stringify({userId:line.userId,displayName:line.displayName,pictureUrl:line.pictureUrl}));
  return line;
...
async function verifyLineIdentity(bindToken=''){
 ...
 if(!session){
  const{error}=await sb.auth.signInAnonymously({options:{data:{name:lp.displayName||'LINE Member',restaurant_slug:'baan-tonkla',source:'line_oa'}}});
...
function Member({session,profile,lineProfile,reload,setPage,lang}){
 ...
 useEffect(()=>{
  ...
  async function ensureMember(){
   ...
   if(!result?.ok){
    if(!session){
     const{error}=await sb.auth.signInAnonymously({options:{data:{name:'LINE Member',restaurant_slug:'baan-tonkla',source:'line_oa'}}});
