...
begin
 ...
 from public.lekhub_submissions s join public.lekhub_submission_items i on i.submission_id=s.id and i.tenant_key=v_tenant
 where s.tenant_key=v_tenant and s.round_date=v_round and coalesce(nullif(v_rates->>i.category,'')::numeric,0)>0 and (
 ...
 select count(*),coalesce(sum(reward_amount),0) into v_count,v_total from public.lekhub_activity_rewards where tenant_key=v_tenant and round_date=v_round;
 ...
 into v_winners from (
  ...
  from public.lekhub_activity_rewards r where r.tenant_key=v_tenant and r.round_date=v_round group by r.line_user_id
 ) x;
